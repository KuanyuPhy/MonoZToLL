...
void xAna_ztoee_old(string inputtxtFilename = "/afs/cern.ch/user/k/kuanyu/ZtoLL/conder_script/DY_HT100_tmp.txt", string outputfile = "./ee_DY_HT100_tmp.root")
{
                ...
                */
                ...
                if (recoeeEvent)
                {
                    ...
                    for (int i = 0; i < nTHINJets; i++)
                    {
                        ...
                        f_JetEta.push_back(thisJet->Eta());
                        f_JetPt.push_back(thisJet->Pt());
                        if (thisJet->Pt() <= 30)
                            continue;
                        ...
                        match_dquarknumb->Fill(match_q, eventWeight);
                    ...
                    for (int i = 0; i < indexForPassAK4.size(); i++)
                    {
                        ...
                        for (int j = 0; j < v_indexPassJetTrack[i].size(); j++)
                        {
                            ...
                            // Fill Tree variable
                            //----------------------
                            v_Trackindex.push_back(j);
                            v_Trackdr.push_back(dR);
                            v_Chi3Dlog.push_back(log10(chi3d));
                            v_Chi3D.push_back(chi3d);
                            v_Chi3DlogPaper.push_back(log10(chiP));
                            v_Chi3DPaper.push_back(chiP);
                            v_Chi2Dlog.push_back(log10(chi2d));
                            v_IP2D.push_back(IP2D);
                            v_TrackPT.push_back(THINjetTrackPt[correctjet_index][correcttrack_index]);
                            v_IP2DxTrackPT.push_back(IP2D * THINjetTrackPt[correctjet_index][correcttrack_index]);
                            ...
                            v_TrackEta.push_back(THINjetTrackEta[correctjet_index][correcttrack_index]);
                            //-----------------------
                            ...
                            {
                                v_bflavorIP2D.push_back(IP2D);
                                v_bflavor3Dsig.push_back(chi3d);
                            }
                            else if (THINjetHadronFlavor[correctjet_index] == 4)
                            {
                                v_cflavorIP2D.push_back(IP2D);
                                v_cflavor3Dsig.push_back(chi3d);
                            }
                            else
                            {
                                v_lightIP2D.push_back(IP2D);
                                v_light3Dsig.push_back(chi3d);
                            }
                        } // End of Track loop
                        if (SumOfTrackPt == 0)
                        {
                        ...
                        // v_Median_log3DIPsig.push_back(median_value(v_Chi3Dlog));
                        // v_Mean_log3DIPsig.push_back(mean_value(v_Chi3Dlog));
                        // v_Median_log2DIPsig.push_back(median_value(v_Chi2Dlog));
                        // v_Median_2DIPsig.push_back(median_value(v_IP2D));
                        ...
                        v_N_Trk_cut3Dsig.push_back(Trackmultiplicitycut);
                        v_N_Tracks.push_back(Trackmultiplicity);
                        // v_aph.push_back(alpha);
                        //------------------
                        //     For Fake rate
                        //------------------
                        TLorentzVector *thisJet = (TLorentzVector *)thinjetP4->At(correctjet_index);            
                        v_fakeJetPt.push_back(thisJet->Pt());
                        v_fakeJetEta.push_back(thisJet->Eta());
                        v_fakeJetMass.push_back(thisJet->M());
                        f_thinjetCSV.push_back(thinJetCSV[correctjet_index]);
                        h_jet_csv->Fill(thinJetCSV[correctjet_index], eventWeight);
                        v_fakeJethadronflavor.push_back(THINjetHadronFlavor[correctjet_index]);
                        v_fakeJetpartonflavor.push_back(THINjetPartonFlavor[correctjet_index]);
                        //-------
                        // For dr
                        //-------
                        v_fakeJet_ledLepdr.push_back(thisJet->DeltaR(goodElectrons[0]));
                        v_fakeJet_subLepdr.push_back(thisJet->DeltaR(goodElectrons[1]));
                        v_fakealpha.push_back(alpha);
                        v_fakealpha2.push_back(alpha2);
                        v_fakealpha3.push_back(alpha3);
                        v_fakealpha4.push_back(alpha4);
                        //------------------
                        // Jetmultiplicity
                        //------------------
                        Jetmultiplicity++;
                        v_aph_index.push_back(i);
                        // if (v_N_Tracks.size() != v_fakealpha.size())
                        //{
                        //     cout << "bug" << endl;
                        //}
                    } // End Match Thin Jet index loop
                    h_njet_pass_trks->Fill(v_fakealpha.size(), eventWeight);
                    if (v_fakealpha.size() == 0) // !!!!! Test
                    {
                        continue;
                    }
                    /*
                    if (v_fakealpha.size() < 2)
                    {
                        cout << "event = " << jEntry << endl;
                        cout << "v_fakeJetPt.size() = " << v_fakealpha.size() << endl;
                        for (int i = 0; i < indexForPassAK4.size(); i++)
                        {
                    ...
                    */
                    ...
                    I_nJets = Jetmultiplicity;
                    if (Jetmultiplicity > indexForPassAK4.size())
                    {
                        cout << "bug here" << endl;
                    }
                    // I_minJethadronflavor = THINjetHadronFlavor[indexForPassAK4[v_aph_index[minElementIndex]]];
                    // I_minJetpartonflavor = THINjetPartonFlavor[indexForPassAK4[v_aph_index[minElementIndex]]];
